...
class OtimizadorDadosExistentes:
    ...
    def otimizar(self, alpha_inicial=None, beta_inicial=None, metodo='Nelder-Mead', max_iter=50):
        ...
        print(f"Método: {metodo}, Máximo de iterações: {max_iter}")
        print("-" * 50)
        ...
        def callback(xk):
            ...
            erro_atual = self.funcao_custo(xk)
            if erro_atual < self.melhor_erro:
                self.melhor_erro = erro_atual
            print(f"Iteração {self.iteracao_atual:2d} | α={xk[0]:.3e} | β={xk[1]:.3e} | Erro={erro_atual:.2f} nm")
        ...
        resultado = minimize(
            ...
            method=metodo,
            bounds=bounds,
            callback=callback,  # ADICIONE ESTA LINHA
        ...
        print("-" * 50)
        print(f"✅ Otimização concluída em {self.iteracao_atual} iterações!")
        ...
        print(f"   Melhoramento: {((resultado.fun - self.funcao_custo(x0)) / self.funcao_custo(x0) * 100):+.1f}%")
